$(document).ready(function() {
  $(".login-bg").backstretch(["/global/img/login-1.jpg", "/global/img/login-2.jpg"], {
    fade: 1500,
    duration: 5000
  });
    ...
    if (validarFomularioLogin()) {
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.sucesso) {
            window.location = "/usuarios/entrar/googleAuthenticator";
          } else {
  ...
  $("#form-google-authenticator").submit(function(event) {
    event.preventDefault();
    ...
    if (validarFomularioLogin()) {
      ...
      $.ajax({
        url: "/usuarios/entrar/googleAuthenticator",
        type: "post",
        data: $("#form-google-authenticator").serialize(),
        success: function(response) {
          if (response.sucesso) {
            window.location = "/";
          } else {
        ...
        error: function(jqXHR, textStatus, errorThrown) {
          $("#div-processando").hide();
          $("#div-acoes").show();
          notificacao("error", "Erro ao validar o código.");
        }
      });
    } else {
      notificacao("warning", "Informe seus dados de acesso.");
