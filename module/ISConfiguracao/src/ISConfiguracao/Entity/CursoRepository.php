...
class CursoRepository extends EntityRepository
{
    ...
    {
        $dql =
            "SELECT cur.id, cur.nome, cur.tipo, cur.status, uni.nome AS unidade" .
    ...
    {
        $dql = "SELECT cur.id, cur.nome" . " FROM ISConfiguracao\Entity\Curso cur" . " WHERE cur.status = :status" . " ORDER BY cur.nome";
